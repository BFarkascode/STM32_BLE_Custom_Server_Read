...
//	I2C1->TIMINGR &= ~(15<<28);					//PRESC value shall be 0x0 for 8 MHz I2CCLK, Fast mode
	...
	HAL_Delay(1);								//We wait for the setup to take effect
}
...
int I2CSCANNER (uint8_t slave_addr) {
	...
	HAL_Delay(1);									//This delay is a bit greater than usual so as to cater to printf commands coming afterwards.
	...
	HAL_Delay(1);									//we need to wait 3 APB cycles after PE is cleared. We wait for 1 us instead.
	return reply;
}
...
int I2CTX (uint8_t slave_addr, uint8_t number_of_bytes, uint8_t *bytes_to_send) {
	...
	//6)We wait for a reply
	HAL_Delay(1);													//waiting. This is to allow the byte and the ACK to cross the bus at 100 kHz
	...
	if ((I2C1->ISR & (1<<4)) == (1<<4)) {										//if the NACKF bit is 1, it means that we had a NO ACKNOWLEDGE from the slave
		...
		while (number_of_bytes)
		{
			...
			number_of_bytes--;
			HAL_Delay(1);										//25 us wait to transmit the byte and receive an ACK. This is for the 100kHz standard clocking
